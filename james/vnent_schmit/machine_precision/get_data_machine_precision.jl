...
# want to load the data for these plots. This might need to run on HPC, but actually might be okay running on this laptop. 

# we want to do it with σ=0.00, 0.001, 0.002. We want to find
# - max bond dim
# - schmitt coefficient values (so we can create a distribution)
# - von neuman entropy
# we want to be able to specify the N values we will use. For now, we will go in increments of 10 from 10 to 80 inclusive
...
function main()
    filename = joinpath(@__DIR__, "simulation_results_mp.jld2")
    for N in N_vals
        for σ in σ_vals
