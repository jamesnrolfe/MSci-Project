...
function main(Min_Nodes, Max_Nodes, Probability, μ, σ, Num_Sweeps, χ, J, Δ)
    Num_Nodes = length(Min_Nodes:Max_Nodes)
    bond_dimension_data = zeros(Num_Nodes, Num_Sweeps)
...
Max_Nodes = 75 #maximum number of spin-1/2 particles
Probability = 1.0 #Probability of a given pair of particles interacting
μ = 1 #mean node weighting
#σ = 0.000 #standard deviation of node weighting
...
surface_dataframe = zeros(Num_Sigmas, Num_Nodes)
#surface_dataframe = DataFrame()
global iteration = 0

for σ in range(Min_Sigma, Max_Sigma, Num_Sigmas)
    global iteration += 1
    average_bond_dataframe = zeros(Num_Nodes, Num_Sweeps)
    average_bond_dataframe = DataFrame(Tables.table(average_bond_dataframe)) #intiallise data collection for each sigma

    for i in 1:data_points #for a given sigma find the bond growth (averaged across N runs, N=data_points)
        output_dataframe = main(Min_Nodes, Max_Nodes, Probability, μ, σ, Num_Sweeps, χ, J, Δ)
        average_bond_dataframe .+= output_dataframe
        println("\n")
    end

    average_bond_dataframe = average_bond_dataframe ./ data_points #find the mean for the runs
    final_bond_dimensions = average_bond_dataframe[:,Num_Sweeps]
    #update the apporpriate area in the results dataframe
    surface_dataframe[iteration, :] = final_bond_dimensions
    #surface_dataframe[!, "Sigma $iteration"] = final_bond_dimensions 
end
...
#print(surface_dataframe)
xaxis = range(Min_Nodes, Max_Nodes)
yaxis = range(Min_Sigma, Max_Sigma, Num_Sigmas)
surface_plot = surface(xaxis, yaxis, surface_dataframe, legend=false)
Plots.savefig(surface_plot,"figures\\surface_$(Min_Nodes)to$(Max_Nodes)_Nodes_sweeps_$(Num_Sweeps)_sigmas_$(Min_Sigma)to$(Max_Sigma)_complete.png")
...
# Plots.savefig(scatter,"figures\\bond_growth_10_to_100_sweeps_30_sigma_0_complete.png")
