...
# ╔═╡ feeafdd0-9e0c-11f0-17e9-9d5ef8867796
begin
	using LightGraphs, Random, PrettyTables
	using Graphs, GraphPlot
	using ITensors, ITensorMPS,LinearAlgebra
end

# ╔═╡ 48cc5f5b-18ad-47bf-9b83-eea609181151
begin
	Nodes = 8
	Probability = 0.9
	Δ = 1
...
# ╔═╡ 94e9bcee-8eae-4231-bf95-92d3bdcfd133
...
# ╔═╡ dc478465-2e0b-4c0f-b7b1-985a5970dd32
begin
	χ = 1000
    ...
    println("DMRG Finished Energy Per Site is ", energy/Nodes)

	#THIS IS WRONG - not consistent with kronecker product ordering of basis:
...
# ╔═╡ 6e5050f1-d091-4c59-a9c6-4bc97e095328
 begin
	⊗(x,y) = kron(x,y) 
 	Hamiltonian = create_XXZ_Ham_matrix(Nodes, adj_mat, Δ)
	vals, vecs = eigen(Hamiltonian)
    #println(vals)
    #pretty_table(vecs)
	en0 = vals[1]
    groundstate = vecs[:,1]
	println("ED Finished Energy Per Site is ", en0/Nodes)
 end

# ╔═╡ 8cc4b2f0-a885-40d1-9337-31aed35772c8
...
function create_XXZ_Ham_matrix(Nodes, adj_mat, Δ)
    ...
    for i in 1:Nodes
        for j in i+1:Nodes
            ...
            H -= adj_mat[i,j]*foldl(⊗, act_pauli_x)
            H -= adj_mat[i,j]*foldl(⊗, act_pauli_y)
            H += Δ*adj_mat[i,j]*foldl(⊗, act_pauli_z)
...
# ╔═╡ b6517dc9-a452-43d9-9be6-6d93e4d4b3d6
DMRG_psi_array'*groundstate

# ╔═╡ b236e259-72d6-4063-87e9-cc66a7b54a9b
pretty_table(reshape(abs.([groundstate;DMRG_psi_array]),2^Nodes,2))

# ╔═╡ 42c96b56-f2e0-4d77-bc3d-915a6e321900
DMRG_psi_array'*DMRG_psi_array

# ╔═╡ 6761d97d-cf50-40c5-bb38-96fc8f3b70d7
groundstate'*groundstate

# ╔═╡ 342d484e-b1af-459f-8f92-f41084cc8c3a
DMRG_psi_array'*groundstate
...
manifest_format = "2.0"
project_hash = "636b5e0c9d6dacb04f2c01f51809e5447def800d"
