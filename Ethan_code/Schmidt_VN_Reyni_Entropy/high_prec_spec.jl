...
Updated for machine precision (1E-16) and configurable maxdim.
"""
function get_schmidt_coeffs(N, σ, J, Δ, μ, num_sweeps, max_bond_dim)
    ...
    # === PRECISION UPDATE ===
    # Allow bond dimension to grow very large to capture full entanglement
    setmaxdim!(sweeps, max_bond_dim) 
    # Set cutoff to machine epsilon for Float64
...
function run_simulation_ent_spec(
    ...
    filename = joinpath(@__DIR__, "high_prec_spec_data_$(σ_val).jld2")
    ...
    for N in N_values
        ...
        Threads.@threads for i in 1:num_graphs_avg
            # Pass max_bond_dim here
            run_coeffs = get_schmidt_coeffs(N, σ_val, J_val, Δ_val, μ, num_sweeps, max_bond_dim)
            
            num_found = length(run_coeffs)
            
            # Store as many coeffs as we found, up to the storage limit
            store_limit = min(num_found, max_coeffs_to_store)
            N_all_coeffs[i, 1:store_limit] = run_coeffs[1:store_limit]
        end

        # Average columns (taking care that zeros imply 'missing' or extremely small values)
        # In high precision, 0.0 usually means the bond dim wasn't reached, so averaging 
        # is technically valid if we assume missing values are zero-weight.
        ...
        try
            jldsave(filename;
                entanglement_spectrum_results, N_values, σ_val, num_graphs_avg, max_coeffs_to_store, max_bond_dim)
...
if isfile(filename)
    ...
    try
        ...
        params_match = (
            ...
            read(loaded_data, "max_bond_dim") == max_bond_dim
        )
