...
function load_and_plot_spectrum()
    ...
    if !isfile(filename)
        ...
        println("Please run 'full_ent_spec.jl' first to generate the data.")
        return
    end
    ...
    for N in N_values_sorted
        coeffs_to_plot = entanglement_spectrum_results[N][1:max_coeffs_to_store]
    ...
    main_plot = plot(plot_list..., 
                     ...
                     size=(1600, 800)) # Set a good size for the 2x4 layout
