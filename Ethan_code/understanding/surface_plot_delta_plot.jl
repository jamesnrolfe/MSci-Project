...
function load_data_and_plot(data_file)
    ...
    for (idx, sigma) in enumerate(sigma_values)
        ...
        plot_title = "Max Bond Dimension χ vs. (N, Δ) (σ = $sigma, J = $J_val)"
        
        # Create the plot for this slice
        [cite_start]plt = plot(N_values, delta_values, data_slice', # Transpose is needed
            st=:surface,
            title=plot_title,
